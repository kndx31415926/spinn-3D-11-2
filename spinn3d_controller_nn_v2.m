...
if ~isfield(gate,'use_task_space_pd'), gate.use_task_space_pd = true; end
...
controller.step       = @step;
    ...
    function varargout = step(t, q, dq, q_ref, dq_ref, robot, kin, pid)
        ...
        % === 1) 特征 + NN 预测（鲁棒） ===
        feat = local_features(controller.featureFcn, t, q, dq, q_ref, dq_ref, robot, kin);
        ...
        if controller.gate.enable
            try
                [~, gH, ~] = spinn3d_energy_gate(q, dq, q_ref, dq_ref, robot, controller.gate);
                if ~isempty(gH), g_H = min(max(gH,0),1); end
            catch, g_H = 1.0;
            end
        end
        if controller.gate.fast_gmin > 0
            dist = local_dist_ee_robot(robot, q, q_ref);   % 用 robot 求距离
        ...
        if controller.gate.use_task_space_pd
            ...
            J = local_J(robot, q); Jlin = J(4:6,:);    % 线速度雅可比
        end
        try, tau_gc = gravityTorque(robot, q); catch, tau_gc = zeros(1,n); end
        tau_dir0 = tau_pd - tau_gc;      % 方向部分单独处理

        % === 5) α 与推进贡献折中（可选） ===
        beta = controller.gate.beta_align;
        if beta > 0
            w = abs(tau_dir0(:)'.*dq(:)');   % 只看方向部分的 -dH 贡献
            if sum(w) > 0
                a_prog = w/sum(w);
