%% spinn3d_run_fast_hit.m
% 一键：Fast‑Hit（首达优先）数据生成 -> 分片落盘 -> 累计主表 -> 训练
% 兼容你原 run_v2 的组织方式（目录/变量名/打印），仿真口径与 simulate 一致（功率在积分前计算 + 逐轴→总功率统一缩放）。
% 依赖：spinn3d_oracle_fast.m, spinn3d_features_v2.m, spinn3d_controller_pid.m,
%       spinn3d_simulate.m, spinn3d_params_block.m, spinn3d_train_fast_hit.m

%% ===== 可配参数 =====
N_TRAJ_ADD     = 4000;       % 本次新增轨迹条数（与刚才保持一致）
CHUNK_TRAJ     = 10;       % 每分片轨迹条数（与原 run_v2 一致）
DO_TRAIN_AFTER = true;     % 生成后立刻训练
DT_GEN         = 0.02;     % 生成用初始 dt（失败会自动降到 0.005 再试）
...
%% ===== 公共参数（由参数块提供） =====
run spinn3d_params_block   % 提供 params, caps, Kp/Ki/Kd, pid_opts, sim, gate 等  :contentReference[oaicite:3]{index=3}
...
for c = 1:num_chunks
    ...
    ds = gen_chunk_fast(simgen, params, caps, gate);  % ds.X (M×D), ds.Y (M×K)
...
function ds = gen_chunk_fast(simgen, params, caps, gate)
    % 生成 simgen.nTraj 条有效轨迹，返回 ds.X (M×D), ds.Y (M×(2n+1))
    [robot, kin] = spinn3d_robot(params);
    robot.DataFormat = 'row';
    robot = sanitize_rbt(robot);   % ★ 体检/修复质量与惯量，解 NaN 根因
    pid   = spinn3d_controller_pid(simgen.Kp, simgen.Ki, simgen.Kd, simgen.pid_opts);  % 方向基线  :contentReference[oaicite:4]{index=4}

    D = 42; n = 4;
    X = zeros(0, D); Y = zeros(0, 2*n+1);
    ...
    while got < want && tries < maxTries
        ...
        % —— 仿真：失败重试（降步长） ——  :contentReference[oaicite:5]{index=5}
        ok = false;
        try
        ...
        for k = 1:numel(log.t)
            ...
            [tau_ref,~] = pid.step(0, qk, dqk, qr, dqr, robot);
            w  = max(0, -tau_ref(:)'.*dqk(:)');
...
% rr: [n×2] in degrees; return 1×n in degrees
    lo = rr(:,1).'; hi = rr(:,2).';
